...
namespace webframe::core {
  ...
  response::response(const std::string &html)
      : response::response(status_line("1.1", "200"), {{"Content-type", "text/html"}},
                 html) {
    rebuild_string();
  }
  response::response(const std::string &http, const std::string &html)
      : response(status_line(http, "200"), {{"Content-type", "text/html"}},
                 html) {
    rebuild_string();
  }
  ...
  response::response(const status_line &s,
  ...
  void response::rebuild_string() {
    ...
    output += std::accumulate(
        std::begin(header), std::end(header), std::string(""),
        [](const std::string &all,
           const std::pair<std::string, std::string> &x) -> std::string {
