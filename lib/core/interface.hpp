...
public:
  application() { app = new _application(); }
  ...
  static constexpr bool init() { return _application::init(); }

  template <typename F> inline application &handle(std::string code, F _res) {
    app->handle(code, _res);
    return *this;
  }

  template <typename Ret, typename... Ts>
  inline application &handle(std::string code,
  ...
  inline application &set_static(const std::string &path,
...
public:
  inline application &extend_with(const router &set_of_routes,
                                  const std::string &prefix = "") {
    app->extend_with(set_of_routes, prefix);
    return *this;
  }

  inline response respond(const std::string &path,
  ...
  inline application &run(const char *PORT, const unsigned int cores,
